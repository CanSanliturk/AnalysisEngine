    ...
    double modulus, double verticalLoadMagnitude, SolverChoice solverChoice)
{
    ...
    membraneRepresentationStream.open("membraneRepresentation.dat");
...
void StrutTieDesign()
{
    ...
    animationStream.open("animation.dat");
...
void OzgunluQuestion()
{
    ...
    // Nodes
    std::map<unsigned int, std::shared_ptr<Node>> nodes;
    auto nodeA = std::make_shared<Node>(1, ptA); nodes[nodeA->NodeIndex] = nodeA;
    auto nodeB = std::make_shared<Node>(2, ptB); nodes[nodeB->NodeIndex] = nodeB;
    auto nodeC = std::make_shared<Node>(3, ptC); nodes[nodeC->NodeIndex] = nodeC;
    auto nodeD = std::make_shared<Node>(4, ptD); nodes[nodeD->NodeIndex] = nodeD;
    auto nodeE = std::make_shared<Node>(5, ptE); nodes[nodeE->NodeIndex] = nodeE;
    ...
    // Material
    auto mat = std::make_shared<Material>(1.0e12, 0.0, 0.0);

    // Members
    std::map<unsigned int, std::shared_ptr<Element>> elements;
    bool isLumpedMassMatrix = true;
    auto elm1 = std::make_shared<TrussMember>(1, nodeA, nodeE, sect1, mat, isLumpedMassMatrix); elements[elm1->ElementIndex] = elm1;
    auto elm2 = std::make_shared<TrussMember>(2, nodeB, nodeE, sect1, mat, isLumpedMassMatrix); elements[elm2->ElementIndex] = elm2;
    auto elm3 = std::make_shared<TrussMember>(3, nodeB, nodeD, sect1, mat, isLumpedMassMatrix); elements[elm3->ElementIndex] = elm3;
    auto elm4 = std::make_shared<TrussMember>(4, nodeD, nodeC, sect1, mat, isLumpedMassMatrix); elements[elm4->ElementIndex] = elm4;
    auto elm5 = std::make_shared<TrussMember>(5, nodeC, nodeA, sect1, mat, isLumpedMassMatrix); elements[elm5->ElementIndex] = elm5;
    auto elm6 = std::make_shared<TrussMember>(6, nodeC, nodeE, sect1, mat, isLumpedMassMatrix); elements[elm6->ElementIndex] = elm6;
    auto elm7 = std::make_shared<TrussMember>(7, nodeE, nodeD, sect1, mat, isLumpedMassMatrix); elements[elm7->ElementIndex] = elm7;

    // Restraints
    std::vector<double> zeros = { 0, 0, 0, 0, 0, 0 };
    std::vector<bool> nodeARestConds = { true, true, true, true, true, true };
    std::vector<bool> nodeBRestConds = { true, false, true, true, true, true };

    std::map<unsigned int, std::shared_ptr<Restraint>> restraints;
    auto rest1 = std::make_shared<Restraint>(nodeA, nodeARestConds, zeros); restraints[1] = rest1;
    auto rest2 = std::make_shared<Restraint>(nodeB, nodeBRestConds, zeros); restraints[2] = rest2;

    // Nodal loads
    std::map<unsigned int, std::shared_ptr<NodalLoad>> nodalLoads;
    double nodalLoadC[6] = { 0, -10e3, 0, 0, 0, 0 };
    double nodalLoadD[6] = { 10e3, 0, 0, 0, 0, 0 };
    auto nlC = std::make_shared<NodalLoad>(nodeC, nodalLoadC); nodalLoads[1] = nlC;
    auto nlD = std::make_shared<NodalLoad>(nodeD, nodalLoadD); nodalLoads[2] = nlD;
    ...
    LOG(" Axial Force on Element 2: " <<  elm2->getAxialForce(disps));
...
void CE583Assignment7_2()
{
    ...
    auto addNode = [&](int nodeIndex, double x, double y, double z) {
        ...
        // Restraint
        if (!(Utils::AreEqual(x, lX / 2.0, x / 10.0) && Utils::AreEqual(y, lY / 2.0, y / 10.0)))
        {
    ...
    // internal energy
    auto internalEnergy = StructureSolver::CalculateInternalEnergy(*str, disps, SolverChoice::Eigen);


    // Print tip displacement
    auto nodalDisp = StructureSolver::GetNodalDisplacements(*nodes[5], disps);

    LOG(" Mid Displacement");
    LOG(" Node Index: " << nodes[5]->NodeIndex);
    LOG(" Node Location: " << nodes[5]->Coordinate.X << " m, " << nodes[5]->Coordinate.Y << " m");
    LOG(" Vertical Displacement: " << nodalDisp(2, 0) << " m");
    LOG(" Inernal Energy: " << internalEnergy << " N.m");
    LOG("");
