...
Matrix<double> StructureSolver::GetDisplacementForStaticCase(Structure& str, SolverChoice solverChoice)
{
    // Define variables
...
Matrix<double> StructureSolver::GetDisplacementsForStaticCaseWithForceVector(Structure& str, Matrix<double>& fVec, SolverChoice solverChoice)
{
    // Define variables
    auto nDofUnrestrained = str.nUnrestrainedDOF;
    auto nDof = str.nDOF;
...
Matrix<double> StructureSolver::GetDisplacementsWithStaticNodalLoads(Structure& str, std::vector<NodalLoad> nodalLoads, SolverChoice solverChoice){
    Matrix<double> fVec(str.nDOF, 1);

    for (auto& nl : nodalLoads) {
        fVec(nl.ActingNode->DofIndexTX - 1, 0) += nl.Loads[0];
        fVec(nl.ActingNode->DofIndexTY - 1, 0) += nl.Loads[1];
        fVec(nl.ActingNode->DofIndexTZ - 1, 0) += nl.Loads[2];
        fVec(nl.ActingNode->DofIndexRX - 1, 0) += nl.Loads[3];
        fVec(nl.ActingNode->DofIndexRY - 1, 0) += nl.Loads[4];
        fVec(nl.ActingNode->DofIndexRZ - 1, 0) += nl.Loads[5];
    }
    
    return StructureSolver::GetDisplacementsForStaticCaseWithForceVector(str, fVec, solverChoice);
}
...
Matrix<double> StructureSolver::GetSupportReactions(Structure& str, Matrix<double>& disps, Restraint& res, SolverChoice solverChoice)
{
    ...
    if (true || solverChoice == SolverChoice::Eigen)
    {
...
Matrix<double> StructureSolver::GetModalPeriods(Structure& str, SolverChoice solverChoice)
{
    // Return value
...
StructureSolver::ImplicitNewmark(Structure& str, std::vector<Matrix<double>> f, double tMax, double dT,
...
Matrix<double> StructureSolver::CalculateMembraneNodalStresses(ShellMember& elm, Matrix<double>& disps, int nodeIndex)
{
...
// This will be moved to shell class
Matrix<double> StructureSolver::CalculatePlateForces(ShellMember& elm, Matrix<double>& disps)
{
    // Thickness
    auto thickness = elm.Thickness;
...
Matrix<double> StructureSolver::CalculateSerendipityPlateForces(SerendipityShell& elm, Matrix<double>& disps)
{
    Matrix<double> reactions(5, 4);
    Matrix<double> bendingMoments(3, 4);
    Matrix<double> shearReactions(2, 1);
...
double StructureSolver::CondenseStiffnessMatrixForSpecificDOF(Structure& str, unsigned int dofIndex, SolverChoice solverChoice)
{
    dofIndex--;
...
double StructureSolver::CondenseForceVectorForSpecificDOF(Structure& str, unsigned int dofIndex, SolverChoice solverChoice)
{
    dofIndex--;
...
double StructureSolver::CalculateInternalEnergy(Structure& str, Matrix<double>& disps, SolverChoice)
{
