...
#pragma region Algorithm
    ...
    //
    // 6- PLACE PREDEFINED STEEL REINFORCEMENT LAYOUT AND PERFORM ANALYSIS WITH REBARS ADDED TO THE SYSTEM.
    //
    // 7- AFTER OBTAINING STRAINS ON STEEL TRUSSES, OPTIMIZE STEEL AREAS TO SET THE STRAINS ON STEEL TRUSSES
    // TO THE PREDEFINED STRAIN LIMIT.
    //
    //
...
#pragma region Input Card
    ...
    auto steelStrainLimit = steelYieldStrain;
    auto steelStressLimit = steelYieldStrength;
...
#pragma region Optimize Steel Areas

    // We know the steel strains at this point. We know rebars and rebar trusses and a single rebar has more than one rebar.
    // For a rebar, we know rebar trusses associated with that rebar. Find the one with the maximum strain and find area that
    // limits its strain to the limit strain.
    std::ofstream rebarAreas;
    rebarAreas.open("plot\\rebarAreas.txt");
    std::map<int, double> rebarAreaMultiplierMap;
    for (auto rebarIndex : rebarToTrussMap)
    {
        LOG(" Rebar #" << rebarIndex.first);
        auto maxRatio = 0.0;
        for (auto rebarTrussIndex : rebarIndex.second)
        {
            auto rebarTruss = dynamic_cast<TrussMember*>(&*str->Elements->at(rebarTrussIndex));
            ...
            auto stress = axialForce / rebarTruss->TrussSection->Area;
            auto strain = stress / rebarTruss->TrussMaterial->E;
            auto ratio = abs(stress) / steelStressLimit;
            maxRatio = maxRatio < ratio ? ratio : maxRatio;
            LOG(" " << ratio);
            // Cool trick for branchless programming. It's hard to follow though, so I commented it out. But it is still cool.
            //maxStiffnessMultipler = ((maxStiffnessMultipler < ratio) * ratio) + ((maxStiffnessMultipler > ratio) * maxStiffnessMultipler);
        }

        // After finding maximum ratio of rebar truss stress to rebar stress limit for a rebar, multiply areas of areas of rebars by the
        // maximum ratio of rebar truss to rebar stress limit for that rebar.
        // First, write data to txt file.
        auto rebarDiameter = std::get<2>(steelReinforcements[rebarIndex.first]);
        rebarAreas << "Rebar #" << rebarIndex.first << "\n";
        rebarAreas << "I-End: " << std::get<0>(steelReinforcements[rebarIndex.first]).X << ", " << std::get<0>(steelReinforcements[rebarIndex.first]).Y << "\n";
        rebarAreas << "J-End: " << std::get<1>(steelReinforcements[rebarIndex.first]).X << ", " << std::get<1>(steelReinforcements[rebarIndex.first]).Y << "\n";
        rebarAreas << "Initial Rebar Diameter: " << rebarDiameter << "\n";
        rebarAreas << "Final Rebar Diameter: " << rebarDiameter * sqrt(maxRatio) << "\n\n";

        // Multiply stiffness of the rebar
        for (auto rebarTrussIndex : rebarIndex.second)
        {
            auto rebarTruss = dynamic_cast<TrussMember*>(&*str->Elements->at(rebarTrussIndex));
            rebarTruss->updateStiffness(maxRatio);
        }
        rebarAreaMultiplierMap[rebarIndex.first] = maxRatio;
        LOG("");
        //LOG(" Max Ratio For Rebar #" << rebarIndex.first << ": " << maxRatio);
    }
    str->updateStiffnessMatrix();
    rebarAreas.close();
...
#pragma region Plot modified rebar stresses

    // Solve displacement
    //auto dispsAfterModifiedRebars = dispsAfterRebars;
    auto dispsAfterModifiedRebars = StructureSolver::CalculateDisplacements(*(str->StiffnessMatrix), *(str->ForceVector), str->nDOF, str->nUnrestrainedDOF, solverChoice);

    // Print midspan results
    midNodeDisplacements = StructureSolver::GetNodalDisplacements(midNode, dispsAfterModifiedRebars);

    LOG(" DISCRETE SYSTEM RESULTS AFTER MODIFIED REBARS");
    LOG(" -------------------------------------------");
    LOG(" Node Location: " << midNode.Coordinate.X << " m, " << midNode.Coordinate.Y << " m");
    LOG(" Vertical Displacement: " << midNodeDisplacements(1, 0) << " m");
    ...
    // Calculate energy of system
    // Calculate total internal energy
    auto internalEnergyOfSystemAfterModifiedRebars = StructureSolver::CalculateInternalEnergy(*str, dispsAfterModifiedRebars, solverChoice);

    LOG(" Internal Energy: " << internalEnergyOfSystemAfterModifiedRebars << "\n");

    // Calculate the strains on the steel rebar trusses and write them to file for visualization.
    std::ofstream finalRebarStrainsDrawer;
    finalRebarStrainsDrawer.open("plot\\finalRebarStrains.dat");
    finalRebarStrainsDrawer << "BEGIN SCENE\n";

    // Draw members bounding box
    drawBoundingBox(finalRebarStrainsDrawer, shape);

    for (auto rebarIndex : rebarToTrussMap)
    {
        LOG(" Rebar Index: " << rebarIndex.first);
        for (auto rebarTrussIndex : rebarIndex.second)
        {
            auto rebarTruss = dynamic_cast<TrussMember*>(&*str->Elements->at(rebarTrussIndex));

            auto axialForce = rebarTruss->getAxialForce(dispsAfterModifiedRebars);
            //auto stress = axialForce / (rebarTruss->TrussSection->Area);
            auto stress = axialForce / (rebarTruss->TrussSection->Area * rebarAreaMultiplierMap[rebarIndex.first]);
            auto strain = stress / rebarTruss->TrussMaterial->E;
            ...
            drawTruss(finalRebarStrainsDrawer, rebarTruss, colour);
        }
        LOG("");
    }

    finalRebarStrainsDrawer << "END SCENE\n";
    finalRebarStrainsDrawer.close();
...
void StrutTieDesign()
{
    ...
    auto isExponenetialModification = false; // stiffness modification factor
